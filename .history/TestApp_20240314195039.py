...
class ImageEditor(QMainWindow, Ui_MainWindow):
    ...
    def set_noise_stacked_widget(self):
        ...
        if pressed_radio.isChecked():
            if pressed_radio.text() == "None":
                ...
                self.stackedWidget.setCurrentIndex(self.slider_map_noise[pressed_radio])
    ...
    def set_radio_button_connections(self):        
        ...
        for noise_radio in [self.radio_uniform, self.radio_gaus, self.radio_sp]:
            noise_radio.toggled.connect(self.set_noise_stacked_widget)
