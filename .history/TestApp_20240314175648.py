...
class ImageEditor(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        (self.item_filter_input, self.img_item_output, self.img_item_greyscale) = (pg.ImageItem() for i in range(3))
    ...
    def setup_plotwidgets(self):
        ...
        for plotWidget, imgItem in zip([self.wgt_input_img, self.wgt_input_img_greyscale, self.wgt_output_img],
                                       [self.item_filter_input, self.img_item_greyscale, self.img_item_output]):
